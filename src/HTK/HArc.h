...
struct _CorrN{
  ...
  BooleanC IsSilence; /* Probably just start & end. */
...
typedef struct _Acoustic{
  ...
  BooleanC SP; /*short pause.  If (SP) rest of variables are NULL.*/
  DVector alphat; /* 1..Nq */
...
typedef struct ArcInfoStruct{
  int nLats;
  BooleanC numLatIncluded; /*if TRUE and this is a denominator lat, means the num lat
