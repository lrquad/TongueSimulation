...
void InitVQ(void)
{
   ...
   numParm = GetConfig("HVQ", TRUEC, cParm, MAXGLOBS);
   if (numParm>0){
...
static VQNode GetNode(Source *src, CovKind ck, short width)
{
   ...
   case INVDIAGC:
      ...
      if (!ReadVector(src, cov.var, FALSEC))
   ...
   case FULLC:
      ...
      if (!ReadTriMat(src, cov.inv, FALSEC))
...
static void StoreTree(FILE *f, VQNode n, CovKind ck, short s)
{
   if (n != NULL){
      ...
      WriteVector(f,n->mean,FALSEC);
      switch(ck){
      case NULLC:   
         break;
      case INVDIAGC:
         WriteVector(f,(Vector)n->cov.var,FALSEC);
         break;
      case FULLC:
         WriteTriMat(f,(TriMat)n->cov.inv,FALSEC);
         break;
      }
      fprintf(f,"\n");
