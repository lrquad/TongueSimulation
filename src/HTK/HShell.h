...
typedef enum {FALSEC=0, TRUEC=1} BooleanC;
...
typedef struct {     /* Defines a source file with position tracking */
   ...
   FILE *f;             /* input stream */
   BooleanC isPipe;      /* input is a pipe */
   BooleanC pbValid;     /* true if putback holds char */
   BooleanC wasQuoted;   /* true if ReadString returned quoted string */
   BooleanC wasNewline;  /* true if SkipWhiteSpace went over newline */
...
typedef struct {     /* Configuration Parameter */
   ...
   ConfVal val;         /* value */
   BooleanC seen;        /* set true when read by any module */
} ConfParam;
...
BooleanC ReadStringWithLen(Source *src, char *s, int buflen); /* With specified length of buffer*/
