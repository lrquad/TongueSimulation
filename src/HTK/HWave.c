...
static BooleanC natReadOrder = FALSEC;    /* Preserve natural read byte order*/
...
typedef struct _Wave{   /* Internal wave file representation */
   ...
   BooleanC isPipe;      /* Source is a pipe */
...
void InitWave(void)
{
   ...
   numParm = GetConfig("HWAVE", TRUEC, cParm, MAXGLOBS);
   if (numParm>0){
...
static BooleanC MustSwap(SrcOrder so)
{
   char bos[MAXSTRLEN];
...
static long NumberBytes(FILE *f, int hSize, BooleanC isPipe)
{
   long fileLen,pos;
...
static long GetAIFFHeaderInfo(FILE *f, Wave w, InputAction *ia)
{
   ...
   BooleanC hasCC=FALSEC, hasSC=FALSEC;
   ...
   *ia = (InputAction) 0;
   ...
   while (!(hasCC && hasSC)) {
      ...
      if (ch1.id == ccid){  /* common chunk found */
         hasCC=TRUEC; commchunk1=ch1;commchunk2=ch2;
      }
      if (ch1.id == scid){  /* sound chunk found */
         hasSC=TRUEC; sndStart=fPtr;
      }
      fPtr += ch1.size + 8;
   }
...
/* EXPORT->ReadEsignalHeader: Get header from Esignal file; return FALSE
   in case of failure. */
BooleanC ReadEsignalHeader(FILE *f, long *nSamp, long *sampP, short *sampS,
                          short *kind, BooleanC *bSwap, long *hdrS,
                          BooleanC isPipe)
{
...
BooleanC ReadHTKHeader(FILE *f, long *nSamp, long *sampP, short *sampS, 
                      short *kind, BooleanC *bSwap)
{
   HTKhdr hdr;
   int n = sizeof hdr;
                    ...
                    short kind, BooleanC *bSwap)
{
   ...
   if (!natWriteOrder && vaxOrder){
      ...
      SwapShort(&hdr.sampSize);
      SwapShort(&hdr.sampKind);
      if (bSwap!=NULL) *bSwap=TRUEC;
   }
   else
      if (bSwap!=NULL) *bSwap=FALSEC;
                   ...
                   HTime frPeriod, HTime *sampPeriod)
{
   ...
   HTime t;
   BooleanC isEXF;	  /* File name is extended */
