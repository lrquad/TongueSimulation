...
static BooleanC beamPruneArcs = TRUEC; /* apply beam pruning to arcs (rather than just nodes) */
static BooleanC compressMerge = TRUEC; /* compressing lattice scores when merging duplicates */
...
BooleanC ScanLLF (LLFInfo *llf, char *fn, char *ext)
{
   char buf[MAXFNAMELEN];
...
void InitLat(void)
{
   ...
   nParm = GetConfig("HLAT", TRUEC, cParm, MAXGLOBS);
   if (nParm>0){
...
*/
BooleanC LatTopSort (Lattice *lat, LNode **topOrder)
{
   int time = -1;       /* new numbering will start at 0 */
   int i;
   LNode *ln;
   LArc *la;
   BooleanC isDAG = TRUEC;
...
{
   startWord = GetLabId (start ? start : "!SENT_START", TRUEC);
   startLMWord = GetLabId (startLM ? startLM : "<s>", TRUEC);
   endWord = GetLabId (end ? end : "!SENT_END", TRUEC);
   endLMWord = GetLabId (endLM ? endLM : "</s>", TRUEC);
   nullWord = GetLabId ("!NULL", TRUEC);
}
...
void FixBadLat (Lattice *lat)
{
   LNode *ln;
   LArc *la;
   BooleanC seenSE, seenOther;
   Word end;

   LatCheck (lat);

   end = GetWord (lat->voc, endWord, FALSEC);
   if (!end)
...
*/
BooleanC ToMergeLatNodesForw(Lattice *lat, LNode *ln)
{
   BooleanC isFound = FALSEC;
   LArc *la1, *la2;
...
/*
  ...
  a node and other active nodes that are connected to this node
*/
BooleanC ToMergeLatNodesBackw(Lattice *lat, LNode *ln)
{
   BooleanC isFound = FALSEC;
   LArc *la1, *la2;
...
*/
Lattice *MergeLatNodesArcs(Lattice *lat, MemHeap *heap, BooleanC mergeFwd)
{
   int i, nn, na;
   LNode *ln, *newln;
   LArc *la, *newla;
