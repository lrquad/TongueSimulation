...
static BooleanC PhoneMEE = TRUEC;      /*IMPORTANT*/           /* If true and the MPE routines are called, do MPE, else MWE (word level) */
static BooleanC CalcAsError = FALSEC;   /* if TRUE, new way of calc'ing error... */ 
static BooleanC PhoneMEEUseContext = FALSEC;                  /*Compare phones-in-context.  I doubt you would want this true, anyway it makes little difference.*/
static BooleanC ExactCorrectness=FALSEC; /*IMPORTANT*/         /* Do 'exact' version of MPE/MWE, not using approximation.  This is slightly better for
...
static BooleanC DoingFourthAcc=FALSEC;    /* Indicate currently it is doing MPE with MMI prior */
...
static BooleanC NoSilence = FALSEC;                      /* If TRUE, then (in non-exact MPE) the silences are omitted from the reference transcription
...
static BooleanC Quinphone = FALSEC;            /* Set this TRUE if this is a quinphone model set.  The code for quinphones depends on the naming conventions 
...
int GetNoContextPhone(LabId phone, int *nStates_quinphone/*actually,number of HMMs per phone*/, int *state_quinphone, HArc *a, int *frame_end){ 
 ...
 get_id:
   ...
   if(len > sizeof(int)) return (int) GetLabId(tmp, TRUEC);
   else return ans;
}
...

   {
	 ...
	 for(a=fbInfo->aInfo->start;a;a=a->foll){  /* first do all arcs for which calcArc==NULL 
            ...
            if(a->calcArc) a->mpe->correctness = 0; /* probably redundant. */
               ...
               BooleanC ZeroCorrectness=FALSEC;
               LabId phone = a->phone;
...
#ifdef SUPPORT_QUINPHONE
               if(Quinphone && quinphone_nstates>1 && quinphone_state > 2){ ZeroCorrectness = TRUEC; }
#endif
...
float GetFinalError(float curr_total, float curr_corr, BooleanC hyp_is_sil){  /* re the "CALCASERROR" option, other version of approx MPE. */
...
float GetLowestNegError(int tStart, int tEnd, int tCurr, float curr_corr, float curr_total,  CorrectArcList **correctArc, int iphone, int *compute_count, BooleanC hyp_is_sil){
  float best = -100,tmp;
...
BooleanC CachingInitialised = FALSEC;
			 ...
			 DVector aqt1, DVector gqt)
{
   ...
   WtAcc *wa, *wammi=NULL;
   PreComp *pMix;
   BooleanC mmix=FALSEC;  /* TRUE if multiple mixture */
   float wght=0;
...
void FBLatFirstPass(FBLatInfo *_fbInfo, FileFormat dff, char * datafn, char *datafn2, Lattice *MPECorrLat){
   int q,T2=0; BooleanC MPE;
  
   fbInfo = _fbInfo;
   if(fbInfo->InUse) FBLatClearUp(fbInfo); 
   fbInfo->InUse=TRUEC; /* will now initialise */
...
void InitFBLat(void)
{
   ...
   for(iter=0;iter<=1;iter++){
      if(iter) nParm = GetConfig("HFBLAT", TRUEC, cParm, MAXGLOBS);
      else nParm = GetConfig("HFWDBKWDBLAT", TRUEC, cParm, MAXGLOBS); /* Backward compatibility */
      ...
      if (nParm>0){
         ...
         if (GetConfBool(cParm,nParm,"MWE",&b)) if(b) PhoneMEE=FALSEC; /* this config also used in HExactMPE.c */
...
void SetDoingFourthAcc(BooleanC DO, int indx){
   DoingFourthAcc = DO;
   add_index = indx;
}
