...
static BooleanC ldBinary = TRUEC;        /* load/dump in binary */

BooleanC strmProj = FALSEC; 
...
void InitTrain(void)
{
   ...
   nParm = GetConfig("HTRAIN", TRUEC, cParm, MAXGLOBS);
   if (nParm>0){
...
static void InitClustering(MemHeap *x, Sequence vpool, int nc,
                           BooleanC treeCluster, CovKind distck, CovKind clusck, Covariance distcov)
{
   int i,numClust;
   Vector v;
   Covariance cov;
                        ...
                        CovKind dck, CovKind cck, Covariance dcov)
{
   ...
   InitClustering(x,vpool,nc,FALSEC,dck,cck,dcov);
   if (trace&T_CGE)
   ...
   for (c=2; c<=ccs->numClust; c++) {
      ...
      for (iter=0,converged=FALSEC; !converged; iter++){
                        ...
                        CovKind dck, CovKind cck, Covariance dcov)
{
   ...
   InitClustering(x,vpool,nc,TRUEC,dck,cck,dcov);
   if (trace&T_CGE)
...
{ 
   ...
   /* Do weights. */
   NewHMMScan(hset,&hss);
   while(GoNextState(&hss,FALSEC)){ /*skip over hmm boundaries.*/
      while(GoNextStream(&hss,TRUEC)){ /*Don't skip over state boundaries.*/
