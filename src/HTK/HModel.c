...
static int nParm = 0;
static BooleanC checking   = TRUEC;       /* check HMM defs */
static BooleanC saveBinary = FALSEC;      /* save HMM defs in binary */
static BooleanC saveGlobOpts = TRUEC;     /* save ~o with HMM defs */
static BooleanC saveRegTree = FALSEC;     /* save regression classes and tree */ 
static BooleanC saveBaseClass = FALSEC;   /* save base classes */ 
static BooleanC saveInputXForm = TRUEC;   /* save input xforms with models set */
static BooleanC forceHSKind= FALSEC;      /* force HMM Set Kind */
static BooleanC keepDistinct=FALSEC;      /* keep orphan HMMs distinct */
static BooleanC discreteLZero=FALSEC;     /* map DLOGZERO to LZERO */
static BooleanC reorderComps=FALSEC;      /* re-order mixture components (PDE) */

static BooleanC allowOthers=TRUEC;        /* allow unseen models in files */
...
static BooleanC indexSet = FALSEC;        /* have the indexes been set for the model set */
...
void InitModel(void)
{
   ...
   InitSymNames();
   nParm = GetConfig("HMODEL", TRUEC, cParm, MAXGLOBS);
   if (nParm>0){
...
typedef struct {     /* Token returned by the scanner */
   ...
   BooleanC binForm;     /* binary form of keyword symbol */
...
static void OWarn(HMMSet *hset,BooleanC equal,char *opt)
{
...
static MILink FindMMF(HMMSet *hset, char *fname, BooleanC ignorePath)
{
   MILink p;
...
MILink AddMMF(HMMSet *hset, char *fname)
{
   ...
   if ((p = FindMMF(hset,fname,FALSEC)) != NULL) 
      return(p);
   p = (MILink)New(hset->hmem,sizeof(MMFInfo));
   ++hset->numFiles;
   p->isLoaded = FALSEC; p->next = NULL;
...
ReturnStatus GetTiedWeights(Source *src, Token *tok, int M, Vector tpdf)
{
   ...
   for (m=1; m<=M; m++) {
      if (repCount>0)         /* get mixture weight */
         ...
         if (tok->binForm) {
            if (!ReadFloat(src,&weight,1,TRUEC)){
            ...
            if (!ReadFloat(src,&weight,1,FALSEC)){
               HMError(src,"Discrete Weight expected");
               return(FAIL);
            }
            c=GetCh(src);
            if (c == '*') {
               if (!ReadShort(src,&repCount,1,FALSEC)){
...
ReturnStatus GetDiscreteWeights(Source *src, Token *tok, int M, ShortVec dpdf)
{
   ...
   for (m=1; m<=M; m++) {
      if (repCount>0)         /* get mixture weight */
         ...
         if (tok->binForm) {
            if (!ReadShort(src,&weight,1,TRUEC)){
               HMError(src,"Discrete Weight expected");
               return(FAIL);
            }
            if (weight<0) {
            ...
            if (!ReadShort(src,&weight,1,FALSEC)){
               HMError(src,"Discrete Weight expected");
               return(FAIL);
            }
            c=GetCh(src);
                             ...
                             int M, int s, Vector tpdf)
{
   ...
   if (isNew) {
      ...
      for (m=1; m<=M; m++){
         ...
         if((mid = GetLabId(macName,FALSEC)) == NULL){
...
{
   int i;
   static BooleanC isInitialised = FALSEC;
...
static LinXForm* GetLinXForm(HMMSet *hset, Source *src, Token *tok)
{
  ...
  if (tok->sym == VECSIZE) {
    ...
    xf->xform = (MatrixC *)New(hmem,(numBlocks+1)*sizeof(MatrixC));
    if(GetToken(src,tok)<SUCCESS){
      HMError(src,"GetToken failed");
      return(NULL);
    }
...
static void PutSymbol(FILE *f, Symbol sym, BooleanC binary)
{
   if (binary){
      fputc(':',f);fputc(sym,f);
   } else
...
static BooleanC putWtActive;   /* false until a weight is output */

/* PutTiedWeight: output mix weight via 1-item buffer */
void PutTiedWeight(FILE *f, short repeatLast, float w, BooleanC binary)
{
...
void PutTiedWeights(FILE *f, StreamElem *se, BooleanC binary)
{
   int repCount=0;
   float weight= -1;
   int m,M;
...
void PutMixWeight(FILE *f, short repeatLast, short w, BooleanC binary)
{
   ...
   if (putWtActive)  {        /* not 1st time so output last w */
      ...
      putWtActive = TRUEC; putWtLCount = 0;
   }
   putWtCurrent = w;
...
void PutDiscreteWeights(FILE *f, StreamElem *se, BooleanC binary)
{
   int repCount=0;
   short weight= -1;
   int m,M;
...
void PutTiedMixtures(HMMSet *hset,FILE *f,int s,StreamElem *se,BooleanC binary)
{
...
void PutDiscrete(FILE *f, StreamElem *se, BooleanC binary)
{
...
static void PutMean(HMMSet *hset, FILE *f, MLink q, SVector m, 
                    BooleanC inMacro, BooleanC binary)
{
   int nUse;
   short size;
...
static void PutCovar(HMMSet *hset, FILE *f, MLink q, STriMat m,
                     Symbol sym, BooleanC inMacro, BooleanC binary)
{
   int nUse;
   short size;
...
static void PutRegNode(HMMSet *hset, FILE *f, RegNode *rnode, BooleanC binary) 
{
   int size,c;
                       ...
                       BooleanC inMacro, BooleanC binary) 
{
   ...
   PutBaseClass(hset,f,NULL,t->bclass,FALSEC,binary);
                      ...
                      BooleanC inMacro, BooleanC binary)
{
   ...
   if (mp->nUse == 0 || inMacro){
      PutMean(hset,f,NULL,mp->mean,FALSEC,binary);
      switch(mp->ckind){
      case DIAGC:  PutVariance(hset,f,NULL,mp->cov.var,FALSEC,binary); break;
      case INVDIAGC: HError(7021,"PutMixPDF: Cannot output INVDIAGC covariance"); break;
      case FULLC:  PutCovar(hset,f,NULL,mp->cov.inv,INVCOVAR,FALSEC,binary); break;
      case LLTC:   PutCovar(hset,f,NULL,mp->cov.inv,LLTCOVAR,FALSEC,binary); break;
      case XFORMC: PutTransform(hset,f,NULL,mp->cov.xform,FALSEC,binary); break;
                         ...
                         BooleanC inMacro, BooleanC binary)
{
   ...
   if (si->nUse == 0 || inMacro){
      ...
      if (si->weights != NULL)
         PutSWeights(hset,f,NULL,si->weights,FALSEC,binary);
      se = si->pdf+1;
      for (s=1; s<=S; s++,se++){
         if (S>1){
         ...
         case SHAREDHS:
            ...
            for (m=1; m<=se->nMix; m++,me++){
               ...
               if (wt > MINMIX) {
                  ...
                  PutMixPDF(hset,f,NULL,me->mpdf,FALSEC,binary);
               }
            }
            break;
         }
      ...
      if (hset->dkind!=NULLD && si->dur != NULL)
         PutDuration(hset,f,NULL,si->dur,FALSEC,binary);
   }
}
			  ...
			  BooleanC inMacro, BooleanC binary)
{
  ...
  if (xf->nUse == 0 || inMacro){
    ...
    if (xf->bias != NULL) {
      ...
      if (!binary) fprintf(f,"\n");
      PutBias(hset,f,NULL,xf->bias,FALSEC,binary);      
    }
    if ( xf->det != 0) {
    ...
    for (i=1;i<=IntVecSize(xf->blockSize);i++) {
      ...
      if (!binary) fprintf(f,"\n");
      PutTransform(hset,f,NULL,xf->xform[i],FALSEC,binary);
    }
    if (xf->vFloor != NULL)
       PutVariance(hset,f,NULL,xf->vFloor,FALSEC,binary);  
  }
}
			  ...
			  BooleanC inMacro, BooleanC binary)
{
  ...
  if (xformSet->nUse == 0 || inMacro){
    ...
    for (i=1;i<=xformSet->numXForms;i++) {
      ...
      WriteInt(f,&i,1,binary);
      if (!binary) fprintf(f,"\n");
      PutLinXForm(hset,f,NULL,xformSet->xforms[i],FALSEC,binary);
    }
  }
}
			  ...
			  BooleanC inMacro, BooleanC binary)
{
  ...
  if (xf->nUse == 0 || inMacro){
    ...
    if (!binary) fprintf(f,"\n");
    PutSymbol(f,LINXFORM,binary);
    PutLinXForm(hset,f,NULL,xf->xform,FALSEC,binary);
  }
}
			  ...
			  BooleanC inMacro, BooleanC binary)
{
  ...
  if (xform->nUse == 0 || inMacro){
    ...
    PutSymbol(f,BASECLASS,binary);
    PutBaseClass(hset,f,NULL,xform->bclass,FALSEC,binary);
    if (xform->parentXForm != NULL) {
      PutSymbol(f,PARENTXFORM,binary);
      PutAdaptXForm(hset,f,NULL,xform->parentXForm,FALSEC,binary);
    }
    PutSymbol(f,XFORMSET,binary);
    if (!binary) fprintf(f,"\n");
    PutXFormSet(hset,f,NULL,xform->xformSet,FALSEC,binary);
...
static void PutOptions(HMMSet *hset, FILE *f, BooleanC binary)
{
   short i,S;
   char buf[64];
   ...
   if ((saveInputXForm) && (hset->xf != NULL)) {
     ...
     PutInputXForm(hset,f,NULL,hset->xf,FALSEC,binary);          
   }
...
static void PutHMMDef(HMMSet *hset, FILE *f, MLink m, BooleanC withHdr,
                      BooleanC binary)
{
   short i;
   StateElem *se;
   ...
   for (i=2; i<hmm->numStates; i++,se++){
      ...
      if (!binary) fprintf(f,"\n");
      PutStateInfo(hset,f,NULL,se->info,FALSEC,binary);
   }
   PutTransMat(hset,f,NULL,hmm->transP,FALSEC,binary);
   if (hset->dkind != NULLD && hmm->dur != NULL)
      PutDuration(hset,f,NULL,hmm->dur,FALSEC,binary);
...
BooleanC HasMacros(HMMSet *hset, char * types)
{
   MLink p;
   char buf[2];
   int h;
   
   if (hset->numMacros == 0) return FALSEC;
   if (types!=NULL){
      types[0] = '\0'; buf[1] = '\0';
      for (h=0; h<MACHASHSIZE; h++)
...
void ApplyVFloor(HMMSet *hset)
{
   ...
   do {
      ...
      if (mixFloored == TRUEC) nFloorVarMix++;
   } while (GoNextMix(&hss,FALSEC));
   EndHMMScan(&hss);
...
static BooleanC IsShared(HMMSet *hset)
{
   char types[20];
...
void CreateHMMSet(HMMSet *hset, MemHeap *heap, BooleanC allowTMods)
{
   int s;
   ...
   hset->allowTMods = allowTMods;  hset->optSet = FALSEC;
   ...
   hset->xf = NULL; hset->logWt = FALSEC;
   for (s=1; s<SMAX; s++) {
   ...
   hset->attRegAccs = FALSEC;
   hset->attXFormInfo = FALSEC;
   hset->attMInfo = FALSEC;
   hset->curXForm = NULL;
...
static ReturnStatus CreateHMM(HMMSet *hset, LabId lId, LabId pId)
{
   MLink m;
   HLink hmm;
   BooleanC newMacro=FALSEC; /* for memory clear up*/
...
static ReturnStatus InitHMMSet(HMMSet *hset, char *fname, BooleanC isSingle)
{
   Source src;
   char buf[MAXSTRLEN];
   LabId lId, pId;
   ...
   hset->firstElem = (BooleanC *) New(hset->hmem, sizeof(BooleanC));

   if (isSingle){
      /* fname is a single HMM file to load as a singleton set */
      lId = GetLabId(fname,TRUEC);
      pId = lId;
...
{
   if(InitHMMSet(hset, fname, FALSEC)<SUCCESS){
      ResetHMMSet(hset);
      return(FAIL);
   }
   return(SUCCESS);
...
{
   if(InitHMMSet(hset, hname, TRUEC)<SUCCESS){
      ResetHMMSet(hset);
      return(FAIL);
   }
   return(SUCCESS);
...
static void SaveMacros(FILE *f, HMMSet *hset, short fidx, BooleanC binary)
{
   MLink m;
   int h;
   ...
   if (saveInputXForm) { 
      for (h=0; h<MACHASHSIZE; h++)
		...
		(((fidx == 1) && ((m->fidx == LOADFIDX) || (m->fidx == CREATEFIDX))) ||
		 (m->fidx == fidx)))
               PutInputXForm(hset,f,m,(InputXForm *)m->structure,TRUEC,binary);     
   }
   fprintf(f,"~o\n");
   ...
   for (h=0; h<MACHASHSIZE; h++)
      for (m=hset->mtab[h]; m!=NULL; m=m->next)
         if (m->fidx == fidx)
            ...
            case 'u':
               PutMean(hset,f,m,(SVector)m->structure,TRUEC,binary);     
               break;
            case 'v':
               PutVariance(hset,f,m,(SVector)m->structure,TRUEC,binary); 
               break;
            case 'i':
               PutCovar(hset,f,m,(SMatrix)m->structure,INVCOVAR,TRUEC,binary); 
               break;
            case 'c':
               PutCovar(hset,f,m,(SMatrix)m->structure,LLTCOVAR,TRUEC,binary); 
               break;
		 ...
		 break;
	       */
            case 'w':
               PutSWeights(hset,f,m,(SVector)m->structure,TRUEC,binary); 
               break;
            case 'd':
               PutDuration(hset,f,m,(SVector)m->structure,TRUEC,binary); 
               break;
            case 't':
               PutTransMat(hset,f,m,(SMatrix)m->structure,TRUEC,binary);
               break;
   ...
   for (h=0; h<MACHASHSIZE; h++)    /* then higher levels */
      for (m=hset->mtab[h]; m!=NULL; m=m->next)
         if (m->fidx == fidx && m->type == 'm')
            PutMixPDF(hset,f,m,(MixPDF *)m->structure,TRUEC,binary);
   for (h=0; h<MACHASHSIZE; h++)    /* then higher levels */
      for (m=hset->mtab[h]; m!=NULL; m=m->next)
         if (m->fidx == fidx && m->type == 's')
            PutStateInfo(hset,f,m,(StateInfo *)m->structure,TRUEC,binary);
   for (h=0; h<MACHASHSIZE; h++)    /* then top model level */
      for (m=hset->mtab[h]; m!=NULL; m=m->next)
         if (m->fidx == fidx && m->type == 'h')
            PutHMMDef(hset,f,m,TRUEC,binary); 
   if (saveBaseClass) {   /* then store base classes */
     ...
     */
     for (h=0; h<MACHASHSIZE; h++)  
	   ...
	   PutBaseClass(hset,f,m,(BaseClass *)m->structure,TRUEC,binary);
   }
   if (saveRegTree) { /* then the regression classes */
     for (h=0; h<MACHASHSIZE; h++)  
       for (m=hset->mtab[h]; m!=NULL; m=m->next)
         if (m->fidx == fidx && m->type == 'r')
	   PutRegTree(hset,f,m,(RegTree *)m->structure,TRUEC,binary);
   }
}
...
void CreateXFormMacro(HMMSet *hset,AdaptXForm *xform, char* macroname)
{
  ...
  if (((id = GetLabId(macroname,FALSEC)) != NULL) && 
...
*/
void SaveAllXForms(HMMSet *hset, char *fname, BooleanC binary)
{
  MLink m;
  int h;
  FILE *f;
  ...
  for (h=0; h<MACHASHSIZE; h++)
	...
	switch(m->type){            /* atomic macros first */
	case 'y':
	  PutBias(hset,f,m,(SVector)m->structure,TRUEC,binary);     
	  break;
	case 'x':
	  PutTransform(hset,f,m,(SMatrix)m->structure,TRUEC,binary);
	  break;
	case 'b':
	  if (saveBaseClass)
	    PutBaseClass(hset,f,m,(BaseClass *)m->structure,TRUEC,binary);
  ...
  for (h=0; h<MACHASHSIZE; h++)    /* then higher levels */
	...
	PutLinXForm(hset,f,m,(LinXForm *)m->structure,TRUEC,binary);
  for (h=0; h<MACHASHSIZE; h++)    /* then higher levels */
    for (m=hset->mtab[h]; m!=NULL; m=m->next)
      if (m->fidx == fidx && m->type == 'g')
	PutXFormSet(hset,f,m,(XFormSet *)m->structure,TRUEC,binary);
  for (h=0; h<MACHASHSIZE; h++)    /* then higher levels */
    for (m=hset->mtab[h]; m!=NULL; m=m->next)
  ...
  for (h=0; h<MACHASHSIZE; h++)    /* then top level */
	...
	PutAdaptXForm(hset,f,m,(AdaptXForm *)m->structure,TRUEC,binary);
  FClose(f,isPipe);  
}

/* EXPORT->SaveOneXForm: outputs an individual transform */
void SaveOneXForm(HMMSet *hset, AdaptXForm *xform, char *fname, BooleanC binary)
{
  FILE *f;
  ...
  if (!binary) fprintf(f,"\n");
  PutAdaptXForm(hset,f,NULL,xform,FALSEC,binary);
  FClose(f,isPipe);  
}

/* EXPORT->SaveInputXForm: outputs an individual transform */
void SaveInputXForm(HMMSet *hset, InputXForm *xf, char *fname, BooleanC binary)
{
  FILE *f;
  BooleanC isPipe;
  ...
  if (!binary) fprintf(f,"\n");
  PutInputXForm(hset,f,NULL,xf,FALSEC,binary);
  FClose(f,isPipe);  
}
...
void SaveInOneFile(HMMSet *hset, char *fname)
{
   ...
   p = AddMMF(hset,fname); p->isLoaded = TRUEC;
   for (h=0; h<MACHASHSIZE; h++)
      for (m=hset->mtab[h]; m!=NULL; m=m->next)
         if (m->type != 'l')
   ...
   if ((numHMMs>1 && !keepDistinct) || numMacs>0){
      if (trace&T_ORP)
         printf("FixOrphanMacros: %d mac and %d hmm orphans to home\n",
                numMacs,numHMMs);
      if (hset->numFiles==0){
         p = AddMMF(hset,orphanMacFile); p->isLoaded = TRUEC;
         if (trace&T_ORP)
...
static void ReOrderComponents(HMMSet *hset)
{
   ...
   NewHMMScan(hset,&hss);
   while(GoNextStream(&hss,FALSEC))
      qsort(hss.ste->spdf.cpdf+1,hss.M,sizeof(MixtureElem),gconst_cmp);
   EndHMMScan(&hss);
}

/* EXPORT->SaveHMMSet: save the given HMM set */
ReturnStatus SaveHMMSet(HMMSet *hset, char *hmmDir, char *hmmExt, char *macroExt, BooleanC binary)
{
   FILE *f;
   MILink p;
   char fname[256];
...
BooleanC PDEMOutP(Vector otvs, MixPDF *mp, LogFloat *mixp, LogFloat xwtdet)
{
   int i,vs;
   LogFloat xmm;
