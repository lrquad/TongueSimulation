...
typedef struct _MMFInfo{
   BooleanC isLoaded;       /* true if contents are loaded */
...
typedef struct RegTree {
  ...
  BooleanC valid;       /* is it valid to generate a transform at the root node */
...
typedef struct {       /* occupancy is always used */
  BooleanC accSum;      /* accumulate the sum of the observations */
  BooleanC accSumSq;    /* accumulate sum-squared (diag) of the observations */
  BooleanC accBTriMat;  /* accumulate weighted outer-product of observations */
  RegTree regTree;     /* the regression tree used for transform */
} XFormAccInfo;

typedef union {         
   MatrixC wgt;          /* for interpolated transforms */
...
typedef struct {
  ...
  BooleanC preQual;     /* is this applied prior to qualifiers? */
...
typedef struct _HMMSet{
   ...
   BooleanC *firstElem;     /* first element added to hmem during MakeHMMSet*/
   ...
   BooleanC allowTMods;     /* true if HMMs can have Tee Models */
   BooleanC optSet;         /* true if global options have been set */
   ...
   BooleanC attRegAccs;   /* have the set of accumulates been attached */
   BooleanC attXFormInfo; /* have the set of adapt info been attached */
   BooleanC attMInfo;     /* have the set of adapt info been attached */
   AdaptXForm *curXForm;
   AdaptXForm *parentXForm;
   
   /* Added to support LogWgts */
   BooleanC logWt;       /* Component weights are stored as Logs */
