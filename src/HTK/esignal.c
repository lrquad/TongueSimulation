            ...
            Annot      *annotate)
{
   ...
   if (temp == NULL)
      return FALSEC;             /* Failure to make temp file. */
   ...
   switch (arch)
      ...
      default:
         ...
         okay = FALSEC;          /* Unsupported architecture. */
         break;
      }
   ...
   if (!okay)
      {
         fclose(temp);
         return FALSEC;          /* Failure to write field list. */
      }
   ...
   if (!WritePreamble(architecture, fld_size, rec_size, file))
      return FALSEC;             /* Failure to write preamble. */
   ...
   if (tot != fld_size)
      return FALSEC;             /* I/O error or wrong fld_size. */
           ...
           FILE         *file)
{
   ...
   switch (arch)
      ...
      case ASCII:
         ...
         break;
      default:
         return FALSEC;          /* Unsupported architecture. */
         break;
      }
        ...
        FILE    *file)
{
   ...
   if (j < len - 1 && buf[len-2] == ' ')
      return FALSEC;             /* not right justified */
        ...
        FILE    *file)
{
   ...
   if (buf == NULL)
      return FALSEC;             /* Allocation failure. */
