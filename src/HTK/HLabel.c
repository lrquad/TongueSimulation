...
static BooleanC stripTriPhones = FALSEC;   /* Enable triPhone stripping */
static int transLev = 0;           /* if >0 filter all but specified level */
static int transAlt = 0;           /* if >0 filter all but specified alt */
static BooleanC compatMode = FALSEC;  /* Allow spaces around . or /// */
...
void InitLabel(void)
{
   ...
   numParm = GetConfig("HLABEL", TRUEC, cParm, MAXGLOBS);
   if (numParm>0){
...
LabId GetLabId(char *name, BooleanC insert)
{
   int h;
   NameCell *p;
...
BooleanC ReadLabel(FILE *f, char *buf)
{
   int c;
   
   c = fgetc(f);
   while (isspace(c)) c = fgetc(f);
   if (c==EOF || !isgraph(c)) return FALSEC;
   do {
      *buf++ = c; c = fgetc(f);
...
BooleanC IsNumeric(char *s)
{
   ...
   len = strlen(s)-1;
   if (!(isdigit((int) s[0])||s[0]=='+'||s[0]=='-')) return FALSEC;
   if (!(isdigit((int) s[len]))) return FALSEC;   
   for (i=1; i<len; i++)
...
static void GetTrSym(Source *src, BooleanC htk)
{
   int nxtch;
   BooleanC trSOL;
   ...
   case '.':
      if (curch=='.' && trSOL==TRUEC && mlfUsed>0 && htk) {
         nxtch = GetCh(src);
...
static void LoadESPSLabels(MemHeap *x, Transcription *t, Source *src)
{
   ...
   while ( trSym == TRNUM ) {    
      ...
      GetTrSym(src,FALSEC);       /* Ignore color */
      GetTrSym(src,FALSEC);       /* Get field label for current level */
      if ( trSym != TRSTR) 
...
static BooleanC FindMLFStr(char *s, char **st, char **en)
{
   char *p,*q;
   
   if (s==NULL || *s == '\0') return FALSEC;
   p = strchr(s,'"');
   if (p==NULL || *(p+1)=='\0') return FALSEC;
   q = strchr(p+1,'"');
   if (q==NULL) return FALSEC;
   *st = p; *en = q;
   return TRUEC;
}
...
static BooleanC NoMLFHeader(char *s)
{
   int len;
   char *e;
   ...
   if (len != 7) return TRUEC;
   *(e+1) = '\0';
   return (strcmp(s,"#!MLF!#") != 0);
}

static BooleanC incSpaces;

/* IsDotLine: return true if line contains only a single dot */
static BooleanC IsDotLine(char *s)
{
   int len;
   char *e;
   BooleanC cut;

   len = strlen(s);cut=FALSEC;
   if (s[len-1]=='\n') {
      len--; s[len]=0;
      if ((len>0) && (s[len-1]=='\r')) {
         len--; s[len]=0;
...
void LoadMasterFile(char *fname)
{
   ...
   BooleanC inEntry = FALSEC;   /* ignore ".." within an entry */
   MLFEntry *e;
   FILE *f;
   ...
   while (fgets(buf,1024,f) != NULL){
      if (!inEntry && FindMLFStr(buf,&pst,&pen)) {
         ...
         *pen = '\0';         /* overwrite trailing pattern quote */
         ...
         if (inEntry && IsDotLine(buf)) inEntry = FALSEC;
   }
...
BooleanC IsMLFFile(char *fn)
{
   FILE *f;
   char buf[1024];
   
   if ((f = fopen(fn,"rb")) == NULL) return FALSEC;
   if (fgets(buf,1024,f) == NULL) {
      fclose(f); return FALSEC;
   }
   if (NoMLFHeader(buf)) {
      fclose(f); return FALSEC;
   }
   fclose(f); return TRUEC;
}
...
static FILE * OpenLabFile(char *fname, BooleanC *isMLF)
{
   FILE *f;
   MLFEntry *e;
   char path[1024],name[256],tryspec[1024];
   BooleanC isMatch = FALSEC;
...
ReturnStatus SaveToMasterfile(char *fname)
{
   ...
   if ((nid=GetLabId(buf,FALSEC))!=NULL) {
