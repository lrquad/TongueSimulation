...
void InitDict(void)
{
   ...
   nParm = GetConfig("HDICT", TRUEC, cParm, MAXGLOBS);
   if (nParm>0){
...
Word GetWord(Vocab *voc, LabId wordName, BooleanC insert)
{
   int h;
   Word p;
...
void InitVocab(Vocab *voc)
{
   ...
   for (i=0; i<VHASHSIZE; i++)
      voc->wtab[i] = NULL;
   voc->nullWord = GetWord(voc, GetLabId("!NULL",TRUEC), TRUEC);
   voc->subLatWord = GetWord(voc, GetLabId("!SUBLATID",TRUEC), TRUEC);
   ...
   while (!src->wasNewline) {
      ...
      if (buf[0] == '[' && buf[len-1] == ']') {    /* outsym */
         ...
         if (ptr!=buf) {
            ...
            labels[2+nphones++] = GetLabId(buf,TRUEC);
         }
      }
      SkipWhiteSpace(src);
   }
...
ReturnStatus ReadDict(char *dictFn, Vocab *voc)
{
   ...
   while(nphones>=0){
      word = GetWord(voc,labels[0],TRUEC);
...
{
   FILE *df;
   BooleanC isPipe,withOut,withProbs;
   Word wid, *wlist;
   Pron thisPron;
   float prob;
   char buf[MAXSTRLEN];
   ...
   for (i=0,withOut=withProbs=FALSEC; i< VHASHSIZE; i++)
      for ( wid = voc->wtab[i]; wid != NULL; wid = wid->next ) {
