...
static BooleanC protectStaks = FALSEC;    /* enable stack protection */
...
void InitMem(void)
{
   ...
   numParm = GetConfig("HMEM", TRUEC, cParm, MAXGLOBS);
   if (numParm>0){
                ...
                float growf, size_t numElem, size_t maxElem)
{
   ...
   x->heap = NULL; 
   x->protectStk = (x==&gstack)?FALSEC:protectStaks; 
   RecordHeap(x);
   if (trace&T_TOP){
      switch (type){
...
MatrixC CreateSMatrix(MemHeap *x, int nrows,int ncols)
{
   size_t vsize;
   int *i,j;
   Vector *m;   
...
BooleanC IsTriMat(MatrixC m)
{
   ...
   n=NumRows(m);
   for(i=1;i<=n;i++)
      if (VectorSize(m[i])!=i) return(FALSEC);
   return(TRUEC);
}
...
void FreeSMatrix(MemHeap *x, MatrixC m)
{
   DecUse(m);
   if (GetUse(m) <= 0)
      Dispose(x,(Ptr *)(m) - 2);
