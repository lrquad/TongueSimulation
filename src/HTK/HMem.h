...
typedef struct {
   ...
   BooleanC protectStk;  /*  MSTAK only, prevents disposal below Stack Top */
}MemHeap;
...
typedef float **MatrixC;    /* matrix[1..nrows][1..ncols] */
typedef MatrixC TriMat;     /* matrix[1..nrows][1..i] (lower triangular) */
...
typedef MatrixC SMatrix;    /* shared matrix[1..nrows][1..ncols] */
typedef MatrixC STriMat;    /* shared matrix[1..nrows][1..i] (lower tri) */
...
/* 
   ...
   MHEAP heaps.
*/
MatrixC  CreateMatrix(MemHeap *x,int nrows,int ncols);
...
BooleanC IsTriMat(MatrixC m);
/*
   Return true if m is actually TriMat
*/

int NumRows(MatrixC m);
int NumDRows(DMatrix m);
int NumCols(MatrixC m);
...
void FreeMatrix(MemHeap *x,MatrixC m);
