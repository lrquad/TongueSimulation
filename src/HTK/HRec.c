...
static int trace=0;
static BooleanC forceOutput=FALSEC;
...
struct align
{
   ...
   BooleanC used;        /* Reference to struct by current inst */
...
struct _NetInst
{
   ...
   BooleanC pxd;         /* External propagation done this frame */
   BooleanC ooo;         /* Instance potentially out of order */
...
struct precinfo {
   ...
   BooleanC models;          /* Keep track of model history */
   BooleanC states;          /* Keep track of state history */
...
void InitRec(void)
{
   ...
   nParm = GetConfig("HREC", TRUEC, cParm, MAXGLOBS);
   if (nParm>0){
...
static LogFloat cPOutP(PSetInfo *psi,Observation *obs,StateInfo *si,int id)
{
   ...
   if (pre->id!=id) { /* bodged at the moment - fix !! */
      if ((FALSEC && psi->mixShared==FALSEC) || (psi->hset->hsKind == DISCRETEHS)) {
...
static void ReOrderList(NetNode *node)
{
   ...
   if (node->inst!=NULL?!node->inst->ooo:TRUEC) return;
   node->inst->ooo=FALSEC;
...
static void AttachInst(NetNode *node)
{
   ...
   /*  correct order. */
   inst->ooo=TRUEC;   /* Need keep list in propagation order */
#ifdef SANITY
   inst->ipos=pri->ipos++;
   pri->start_inst=inst;
#endif
...
PSetInfo *InitPSetInfo(HMMSet *hset)
{
   ...
   if (hset->numSharedMix>0) {
      ...
      psi->mixShared=FALSEC,psi->nmp=0,psi->mPre=NULL;
...
static void LatFromPaths(Path *path,int *ln,Lattice *lat)
{
   ...
   nullwordId = GetWord(lat->voc,GetLabId("!NULL",FALSEC),FALSEC);
   splabid = GetLabId(SP,FALSEC);
...
VRecInfo *InitVRecInfo(PSetInfo *psi,int nToks,BooleanC models,BooleanC states)
{
   VRecInfo *vri;
   PreComp *pre;
   int i,n;
...
Lattice *CompleteRecognition(VRecInfo *vri,HTime frameDur,MemHeap *heap)
{
   ...
   if (heap!=NULL) {
      lat=NULL;vri->noTokenSurvived=TRUEC;
...
static BooleanC WordMatch(NBestEntry *cmp,NBestEntry *ans)
{
   if (cmp==ans) return(TRUEC);
   else if (cmp==NULL || ans==NULL) return(FALSEC);
   else if (cmp->larc->end->word!=ans->larc->end->word) return(FALSEC);
...
Transcription *TranscriptionFromLattice(MemHeap *heap,Lattice *lat,int N)
{
   ...
   nullWord=GetWord(lat->voc, GetLabId("!NULL", FALSEC), FALSEC);
   trans=CreateTranscription(heap);
   for (i=1;i<=n;i++) {
      states=models=FALSEC;
...
void FormatTranscription(Transcription *trans,HTime frameDur,
                         BooleanC states,BooleanC models,BooleanC triStrip,
                         BooleanC normScores,BooleanC killScores,
                         BooleanC centreTimes,BooleanC killTimes,
                         BooleanC killWords,BooleanC killModels)
{
   LabList *ll;
   LLink lab;
   HTime end;
      ...
      for (lev=1;lev<=trans->numLists;lev++) {
         ...
         for(lab=ll->head->succ;lab->succ!=NULL;lab=lab->succ) {
            if (states && !models) {
               ...
               lab->labid=GetLabId(buf,TRUEC);
            }
            else {
               strcpy(buf,lab->labid->name);
               TriStrip(buf); lab->labid=GetLabId(buf,TRUEC);
            }
            for (j=1;j<=ll->maxAuxLab;j++) {
               if (lab->auxLab[j]==NULL) continue;
               strcpy(buf,lab->auxLab[j]->name);
               TriStrip(buf); lab->auxLab[j]=GetLabId(buf,TRUEC);
            }
         }
      }
   }
