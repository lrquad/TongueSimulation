...
void lobo::computeBFSDistance(std::vector<QuadraticElement> &sampleNodes_list_, std::vector<FrameBase*> &frame_list_, VectorXd &regioninfo)
{
	for (int i = 0; i < sampleNodes_list_.size(); i++)
	{
		sampleNodes_list_[i].distance_to_frame.resize(frame_list_.size());
		std::fill(sampleNodes_list_[i].distance_to_frame.begin(), sampleNodes_list_[i].distance_to_frame.end(), 0);
	}

	for (int i = 0; i < frame_list_.size(); i++)
	{
		std::cout << "start frame :" << i << std::endl;
		int startelement = frame_list_[i]->ele_index_;
		std::vector<bool> visited_mark;
		visited_mark.resize(sampleNodes_list_.size());
		...
		while (queue_list.size() != 0)
		{
			...
			for (int j = 0; j < sampleNodes_list_[current_ele].neighbor.size(); j++)
			{
				int neighborid = sampleNodes_list_[current_ele].neighbor[j];
				if (neighborid != -1)
				{
					if (!visited_mark[neighborid])
					{
						double distancetocurrent;
						int neighbor_index = neighbormap(j);
						distancetocurrent = edgelength[neighbor_index] * sampleNodes_list_[i].edge_length;
						sampleNodes_list_[neighborid].distance_to_frame[i] = sampleNodes_list_[current_ele].distance_to_frame[i] + distancetocurrent;
...
void lobo::computeDisplacementField(std::vector<QuadraticElement> &sampleNodes_list_, std::vector<FrameBase*> &frame_list_, VectorXd &regioninfo)
{
	std::cout << "start compute weight laplacian ...";
	...
	int n = sampleNodes_list_.size();
	int s = frame_list_.size();
	SparseMatrix<double> L(n, n); // Laplacian matrix  
	std::vector<Triplet<double>> sparseL;

	MatrixXd F(s,n); // constrain matrix
	SparseMatrix<double> A(n + s, n);
	// construct L; L is same and constant
	for (int i = 0; i < sampleNodes_list_.size(); i++)
	{
		...
		for (int j = 0; j < sampleNodes_list_[i].neighbor.size(); j++)
		{
			if (sampleNodes_list_[i].neighbor[j] != -1)
			{
				int neighbor_index = neighbormap(j);
				edgelengthtotal += edgelength[neighbor_index];
				numberofneighbor++;
			}
		}
		//wij 
		for (int j = 0; j < sampleNodes_list_[i].neighbor.size(); j++)
		{
			...
			{
				int neighbor_index = neighbormap(j);
				double wij = edgelength[neighbor_index] / edgelengthtotal;

				sparseL.push_back(Triplet<double>(i, sampleNodes_list_[i].neighbor[j], -wij));
			}
		}
	}
	...
	for (int i = 0; i < frame_list_.size(); i++)
	{
		...
		sparseA.insert(sparseA.end(), sparseL.begin(), sparseL.end());
		VectorXd B(n+s);
		B.setZero();
		B.data()[n + i] = 1;
		for (int j = 0; j < frame_list_.size(); j++)
		{
			sparseA.push_back(Triplet<double>(j + n, frame_list_[j]->ele_index_, 1));
		}
...
#endif
		VectorXd weights = LDLT.solve(d);

		for (int j = 0; j < sampleNodes_list_.size(); j++)
		{
			sampleNodes_list_[j].weights[i] = (weights.data()[j]);
		} 
	}
	std::cout<<"finished."<< std::endl;
}

void lobo::computeWeightGradient(std::vector<QuadraticElement> &sampleNodes_list_, double edgeratio)
{
	std::cout << "start compute gradient ...";
	int framesize = sampleNodes_list_[0].weights.size();

	for (int i = 0; i < sampleNodes_list_.size(); i++)
	{
		sampleNodes_list_[i].weight_gradients.resize(framesize);

		for (int f = 0; f < framesize; f++)
		{
			sampleNodes_list_[i].weight_gradients[f].resize(1, 3);
			double gx, gy, gz;
			double px, nx, py, ny, pz, nz;

			int nx_id = sampleNodes_list_[i].neighbor[4];
			nx = nx_id == -1 ? sampleNodes_list_[i].weights[f] : sampleNodes_list_[nx_id].weights[f];

			int px_id = sampleNodes_list_[i].neighbor[21];
			px = px_id == -1 ? sampleNodes_list_[i].weights[f] : sampleNodes_list_[px_id].weights[f];

			int ny_id = sampleNodes_list_[i].neighbor[10];
			ny = ny_id == -1 ? sampleNodes_list_[i].weights[f] : sampleNodes_list_[ny_id].weights[f];

			int py_id = sampleNodes_list_[i].neighbor[15];
			py = py_id == -1 ? sampleNodes_list_[i].weights[f] : sampleNodes_list_[py_id].weights[f];

			int nz_id = sampleNodes_list_[i].neighbor[12];
			nz = nz_id == -1 ? sampleNodes_list_[i].weights[f] : sampleNodes_list_[nz_id].weights[f];

			int pz_id = sampleNodes_list_[i].neighbor[13];
			pz = pz_id == -1 ? sampleNodes_list_[i].weights[f] : sampleNodes_list_[pz_id].weights[f];

			gx = px - nx;
			gx /= (sampleNodes_list_[i].edge_length * 2 * edgeratio);

			gy = py - ny;
			gy /= (sampleNodes_list_[i].edge_length * 2 * edgeratio);

			gz = pz - nz;
			gz /= (sampleNodes_list_[i].edge_length * 2 * edgeratio);

			sampleNodes_list_[i].weight_gradients[f].data()[0] = gx;
			sampleNodes_list_[i].weight_gradients[f].data()[1] = gy;
			sampleNodes_list_[i].weight_gradients[f].data()[2] = gz;
		}
	}
	std::cout << "finished." << std::endl;
}

int lobo::searchClosedSamplePoint(std::vector<QuadraticMWElement> &sampleNodes_list_, Vector3d &position)
{
	double mindist = (sampleNodes_list_[0].ele_position - position).norm();
	int minindex = 0;
	for (int i = 0; i < sampleNodes_list_.size(); i++)
	{
		if ((sampleNodes_list_[i].ele_position - position).norm() < mindist)
		{
			mindist = (sampleNodes_list_[i].ele_position - position).norm();
			minindex = i;
		}
	}
...
int lobo::searchClosedSamplePoint(std::vector<QuadraticElement> &sampleNodes_list_, Vector3d& position)
{
	double mindist = (sampleNodes_list_[0].ele_position - position).norm();
	int minindex = 0;
	for (int i = 0; i < sampleNodes_list_.size(); i++)
...
void lobo::setRoataionMatrix(Matrix3d &rotation, double angle, double x, double y, double z)
{
	QMatrix4x4 rotation_q;
	rotation_q.setToIdentity();
	rotation_q.rotate(angle, x, y, z);
	
	rotation.data()[0] = rotation_q(0, 0);
	rotation.data()[1] = rotation_q(1, 0);
	rotation.data()[2] = rotation_q(2, 0);
	rotation.data()[3] = rotation_q(0, 1);
	rotation.data()[4] = rotation_q(1, 1);
	rotation.data()[5] = rotation_q(2, 1);
	rotation.data()[6] = rotation_q(0, 2);
	rotation.data()[7] = rotation_q(1, 2);
	rotation.data()[8] = rotation_q(2, 2);
}
