...
int ARPACKSolver::SolveGenEigShInv(SparseMatrix& K, 
	SparseMatrix& M, 
	int numEigenvalues, 
	double *eigenvalues, 
	double *eigenvectors, 
	...
	int verbose)
{
	...
	*****/
// 	SparseMatrix KsigmaM(np, np);
// 	vector<Triplet<double>> nnz_list;
// 	nnz_list.reserve(K.nonZeros() + M.nonZeros());
// 
// 	for (int k = 0; k < M.outerSize(); ++k)
// 	{
// 		for (SparseMatrix::InnerIterator it(M, k); it; ++it)
// 		{
// 			nnz_list.push_back(Triplet<double>(it.row(), it.col(), it.value() * -1.0 * sigma));
// 		}
// 	}
// 
...
// 		{
// 			nnz_list.push_back(Triplet<double>(it.row(), it.col(), it.value()));
// 		}
// 	}
// 
// 	KsigmaM.setFromTriplets(nnz_list.begin(), nnz_list.end());
	SparseMatrix KsigmaM = K - sigma * M;
	//KsigmaM.makeCompressed();
	...
	**/
	...
	cout << "factorize K - sigma * M Matrix..." << endl;

#ifdef EIGEN_USE_MKL_ALL
	PardisoLDLT<SparseMatrix> KsigmaMSolver;
#else
	SimplicialLDLT<SparseMatrix> KsigmaMSolver;
#endif

	KsigmaMSolver.compute(KsigmaM);
	if (KsigmaMSolver.info() != Success)
	{
		cout << "linear solver error!" << endl;
	}
	
	cout << "done" << endl;
	VectorXd temp_vec(np);
	temp_vec.setZero();
	...
	int IPNTR[11];
	double * WORKD = (double*) malloc (sizeof(double) * 3 * N);
	int LWORKL = 2 * (NCV*NCV + 8 * NCV + 100); // at least NCV**2 + 8*NCV; 
	double * WORKL = (double*) malloc (sizeof(double) * LWORKL);
	int INFO = 0;
	...
	do
	{
		...
		switch(IDO)
		{
			case -1:
				...
				//  &WORKD[IPNTR[0]-1] --> output vector
				//  &WORKD[IPNTR[1]-1] --> input vector

				/***
				invKMSolver.ComputeInvMK(&WORKD[IPNTR[0]-1], &WORKD[IPNTR[1]-1]);
				***/
				// here is eigen alternative
				memcpy(temp_vec.data(), &WORKD[IPNTR[0] - 1], sizeof(double)* np);
				temp_vec = M * temp_vec;
				memcpy(&WORKD[IPNTR[0] - 1], temp_vec.data(), sizeof(double)* np);
				temp_vec = KsigmaMSolver.solve(temp_vec);
				memcpy(&WORKD[IPNTR[1] - 1], temp_vec.data(), sizeof(double)* np);
			break;
			...
			case 1:
				...
				***/
				memcpy(temp_vec.data(), &WORKD[IPNTR[2] - 1], sizeof(double)* np);
				temp_vec = KsigmaMSolver.solve(temp_vec);
				memcpy(&WORKD[IPNTR[1] - 1], temp_vec.data(), sizeof(double)* np);
			break;
			...
			case 2:
				...
				/***
				M->MultiplyVector(&WORKD[IPNTR[0]-1], &WORKD[IPNTR[1]-1]);
				&WORKD[IPNTR[0]-1]-->input
				&WORKD[IPNTR[1]-1]-->output
				***/
				memcpy(temp_vec.data(), &WORKD[IPNTR[0] - 1], sizeof(double) * np);
				temp_vec = M * temp_vec;
				memcpy(&WORKD[IPNTR[1] - 1], temp_vec.data(), sizeof(double) * np);
	...
	***/
	...
	for (int i = 0; i < nconv; i++)
	{
		...
		***/
		memcpy(temp_vec.data(), &eigenvectors[np*i], sizeof(double) * np);
		VectorXd mv = M * temp_vec;
		double mass_inner = mv.dot(temp_vec);
		temp_vec = temp_vec / mass_inner;
		memcpy(&eigenvectors[np*i], temp_vec.data(), sizeof(double)* np);
	}
