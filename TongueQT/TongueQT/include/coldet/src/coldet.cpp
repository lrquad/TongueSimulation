                                     ...
                                     float* other_transform)
{
  m_trilist1.clear();
  m_trilist2.clear();
  m_triangleList1.clear();
  m_triangleList2.clear();
          ...
          for(int i=0;i<tnum2;i++)
          {
            ...
            for(int j=0;j<tnum1;j++)
            {
              ...
              if (tt.intersect(*bt1)) 
              {
				...
				m_triangleList1.push_back(m_ColTri1);
				m_triangleList2.push_back(m_ColTri2);
				m_trilist1.push_back(m_iColTri1);
				m_trilist2.push_back(m_iColTri2);
                //return true;
              }
            }
          }
...
bool CollisionModel3DImpl::getCollidingTriangleList(std::vector<int> &trilist1, std::vector<int> &trilist2)
{
	trilist1 = m_trilist1;
	trilist2 = m_trilist2;
	return true;
...
bool CollisionModel3DImpl::getCollisionPoint(float p[3], int triangle1, int traingel2, bool ModelSpace)
{
	Vector3D& v = *((Vector3D*)p);
	switch (m_ColType)
	{
	case Models: v = my_tri_tri_intersect(m_triangleList1[triangle1], m_triangleList2[traingel2]); break;
	case Sphere:
	case Ray:    v = m_ColPoint; break;
	default:     v = Vector3D::Zero;
	}
