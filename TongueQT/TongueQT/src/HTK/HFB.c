...
static BooleanC alCompLevel = FALSEC;   /* align model at component level */
...
static BooleanC pde = FALSEC;  /* partial distance elimination */
static BooleanC sharedMix = FALSEC; /* true if shared mixtures */
...
void InitFB(void)
{
   ...
   for (m = 0; m < 2; ++m) {
      nParm = GetConfig(m==0 ? "HFWDBKWD" : "HFB", TRUEC, cParm, MAXGLOBS);
      if (nParm>0){
...
void UseAlignHMMSet(FBInfo* fbInfo, MemHeap* x, HMMSet *al_hset)
{
   ...
   fbInfo->twoModels = TRUEC;
}

/* Initialise the utterance memory requirements */
void InitUttInfo( UttInfo *utt, BooleanC twoFiles )
{
...
static BooleanC NonSkipRegion(int skipstart, int skipend, int t)
{
   ...
   if (wa->time==t)           /* seen this state before */
	 ...
	 for (m=2,me=ste->spdf.cpdf+2;m<=M;m++,me++) {
	    ...
	    if (wt>LMINMIX){
	       mp = me->mpdf;
	       otvs = ApplyCompFXForm(mp,v,xform,&det,t);
	       if (PDEMOutP(otvs,mp,&mixp,x-wt-det) == TRUEC) {
		  mixp += det;
		  x = LAdd(x,wt+mixp);
	       }
    ...
    /* Check that PLAINHS is handled correctly this  way - efficient? */
		     ...
		     if (S==1)
		        outprobj[0] = ShStrP(hset,ste,ot.fv[s],t,fbInfo->al_inXForm,&ab->abMem);
                        ...
                        if (((WtAcc *)ste->hook)->time==t) seenState=TRUEC;
                        else seenState=FALSEC;
...
static BooleanC StepBack(FBInfo *fbInfo, UttInfo *utt, char * datafn)
{
   LogDouble lbeta;
...
static void UpMixParms(FBInfo *fbInfo, int q, HLink hmm, HLink al_hmm,
                       ...
                       BooleanC twoDataFiles, LogDouble pr)
{
...
BooleanC FBFile(FBInfo *fbInfo, UttInfo *utt, char * datafn)
{
   BooleanC success;
