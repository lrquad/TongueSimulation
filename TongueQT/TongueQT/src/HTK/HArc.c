...
static BooleanC IsLMScale = FALSEC;
static float WDPEN = 0;
static BooleanC IsWdPen = FALSEC;
...
BooleanC StackInitialised=FALSEC;
...
float Depth(ArcInfo *aInfo, BooleanC All){
...
BooleanC BackTransitions(ArcInfo *aInfo){ /* a check, should never happen */
   HArc *a;
   ArcTrans *at;
   for(a=aInfo->start;a;a=a->foll)
...
void ArcFromLat(ArcInfo *aInfo, HMMSet *hset){
   ...
   if(trace&T_ARC && debug++ < 100){
      printf("[HArc:] %d arcs, depth %f, depth[reduced] %f\n", aInfo->nArcs, Depth(aInfo,TRUEC), Depth(aInfo,FALSEC));
      if(trace&T_ARC2)
...
BooleanC SameArcs(LArc *a1, LArc *a2){ /*word arcs have same times & phones */
   int seg;
   if(a1->nAlign!=a2->nAlign) return FALSEC;
   if(a1->start->time!=a2->start->time) return FALSEC;
   for(seg=0;seg<a1->nAlign;seg++) 
      if (a1->lAlign[seg].dur != a2->lAlign[seg].dur || 
          a1->lAlign[seg].label != a2->lAlign[seg].label) return FALSEC;
   return TRUEC; 
}

BooleanC LatInLatRec(LNode *n1, LNode *n2){

   if(n1->foll==NULL && n2->foll==NULL){
      return TRUEC; /*Path found.*/
   }

   if(n1->pred && n1->pred->parc != NULL) return TRUEC; /*This is to take the case where the numerator could have choices of arcs (multiple prons).
                                                         Where the arcs re-join, just check the last one re-joining.  This makes sure we don't take exponential time.*/

   if(n1->foll==NULL || n2->foll==NULL) return FALSEC;
   else{
      LNode *newn1;
      LArc *la;
      LArc *n1FollArc;
      ...
      for(n1FollArc=n1->foll;n1FollArc;n1FollArc=n1FollArc->farc){ /*For each following arc.  This could make the time exponential, were it not for
         ...
         for(la = n2->foll;la;la=la->farc){
            ...
            if(newn2->word == newn1->word  &&
               ...
               if(LatInLatRec(newn1, newn2)) ThisIsOK = TRUEC; /*We've found a path to the end (or to re-join point).  --> this num pron is OK.*/
         }
         if (!ThisIsOK) return FALSEC; /*This num pron was not there.*/
      }
      return TRUEC;
   }
}

BooleanC LatInLat(Lattice *numLat, Lattice *denLat){
...
void InitArc(void)
{
   ...
   nParm = GetConfig("HARC", TRUEC, cParm, MAXGLOBS);
   if (nParm>0){
      if (GetConfInt(cParm,nParm,"TRACE",&i)) trace = i;                          /*   Set TRACE=1 for minimal tracing, 3 for fuller tracing.  */
      if (GetConfFlt(cParm,nParm,"LMSCALE",&f)){ LMSCALE = f; IsLMScale = TRUEC; } /*   Overrides lattice-specified one.  */
      if (GetConfFlt(cParm,nParm,"FRAMEDUR",&f)){ FRAMEDUR = f; }                 /*   Important.  Frame duration in seconds.  If != 0.01, specify it. */
      if (GetConfFlt(cParm,nParm,"WDPEN",&f)){ WDPEN = f; IsWdPen = TRUEC; }       /*   Overrides lattice-specified one.  */
   }
}
