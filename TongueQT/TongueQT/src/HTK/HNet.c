...
*/
BooleanC remDupPron=TRUEC;
/*
   Remove duplicate pronunciations
*/

BooleanC sublatmarkers=FALSEC;
/*
...
void InitNet(void)
{
   ...
   nParm = GetConfig("HNET", TRUEC, cParm, MAXGLOBS);
   if (nParm>0){
...
static void OutputIntField(char field,int val,BooleanC bin,
                           char *form,FILE *file)
{
   fprintf(file,"%c%c",field,bin?'~':'=');
   if (bin)
      WriteInt(file,&val,1,TRUEC);
   else
...
static void OutputFloatField(char field,float val,BooleanC bin,
                             char *form,FILE *file)
{
   fprintf(file,"%c%c",field,bin?'~':'=');
   if (bin)
      WriteFloat(file,&val,1,TRUEC);
   else
      fprintf(file,form,val);
   fprintf(file," ");
}
...
static int ReadAlign(Lattice *lat,LArc *la,char *buf)
{
   ...
   for(i=0,lal=la->lAlign,p=buf;i<n;i++,lal++) {
      ...
      c=*p;*p=0;lal->label=GetLabId(str,TRUEC);
...
Lattice *ReadOneLattice(Source *src, MemHeap *heap, Vocab *voc, 
                               BooleanC shortArc, BooleanC add2Dict)
{
...
#define DBUFLEN 4096
   ...
   if (sublatstr!=NULL) lat->subLatId = GetLabId(sublatstr,TRUEC);
         ...
         while((ptr=GetNextFieldName(nbuf,&del,src)) != NULL) {
            if (nbuf[0]=='\n') break;
               ...
               case 'L':
                  ...
                  if((subLatId=AdjSubList(lat,GetLabId(vbuf,TRUEC),NULL,+1))==NULL) {
...
void CopyLattice(Lattice *lat, Lattice *newlat, 
                 int *newNodes, int *newArcs, BooleanC ignoreNull)
{
   int i,j;
...
static void ExpandLattice(Lattice *newlat, int nNodes, int nArcs)
{
   ...
   for (i=0; i< nNodes; i++) {
      ...
      if (thisNode->word == newlat->voc->subLatWord) {
         ...
         CopyLattice(subLat,newlat,&newNodes,&newArcs,FALSEC);
         if (thisNode->tag != NULL) {
            ...
            if (sublatmarkers) {
               ...
               node->word=GetWord(newlat->voc,GetLabId("!NULL", TRUEC),TRUEC);
...
Lattice *ExpandMultiLevelLattice(MemHeap *heap, Lattice *lat, Vocab *voc)
{
   ...
   newlat->voc = lat->voc;
   newArcs = newNodes = 0;
   CopyLattice(lat,newlat,&newNodes,&newArcs,FALSEC);  /* copy the top level  */
   ...
   newArcs = newNodes = 0;
   CopyLattice(newlat,final,&newNodes,&newArcs,TRUEC); /* remove NULL id nodes */
...
Lattice *LatticeFromLabels(LabList *ll,LabId bnd,Vocab *voc,MemHeap *heap)
{
   ...
   for (i=1,ln=lat->lnodes;i<=N;i++,ln++,n++) {
      ...
      /* Node */
      ln->n=i-1;
      if ((ln->word=GetWord(voc,labid,FALSEC))==NULL)
...
static BooleanC IsWd0Link(NetLink *link)
{
   int i;
   NetNode *nextNode;
...
typedef struct pronholder
{
   ...
   BooleanC fci;     /* Final phone context independent */
   BooleanC tee;     /* TRUE if word consists solely of tee models */
...
static HMMSetCxtInfo *NewHMMSetCxtInfo(HMMSet *hset, BooleanC frcCxtInd)
{
   HMMSetCxtInfo *hci;
   ...
   if (frcCxtInd) {
      ...
      hci->cxs[0]=hci->cis[0]=hci->cfs[0]=GetLabId("<undef>",TRUEC);
   }
...
BooleanC IsHCIContextInd(HMMSetCxtInfo *hci,LabId labid)
{
   int c;
   if (hci->nc==0) return(TRUEC);
...
static BooleanC ClosedDict(Vocab *voc,HMMSet *hset)
{
   Word word;
   Pron pron;
   int i,h;
...
static int DefineContexts(HMMSetCxtInfo *hci)
{
   ...
   hci->nc=0; hci->sLeft=hci->sRight=FALSEC;
...
static BooleanC IsRContextInd(HMMSetCxtInfo *hci,PronHolder *p,int pos,int xlc)
{
   LabId labid;
   HLink hmm,cmp;
   int i,j,lc;
...
static BooleanC InternalDict(Vocab *voc,HMMSetCxtInfo *hci)
{
   Word word;
   Pron pron;
   int i,j,h,lc,rc;
...
{ 
   ...
   pInst->chain=NULL;
   pInst->tee=FALSEC;  /* Empty words are !NULL */
   if (hci->xc>0) {
                           ...
                           Vocab *voc,MemHeap *heap,char *frcSil)
{
   ...
   net->nullWord = GetWord(voc,GetLabId("!NULL", TRUEC),TRUEC);
                    ...
                    Network *net,HMMSetCxtInfo *hci)
{
   ...
   for(j=q-1;j>p;j--) {
      ...
      if (hmm->transP[1][hmm->numStates]<LSMALL) pInst->tee=FALSEC;
                          ...
                          Network *net,HMMSetCxtInfo *hci,MemHeap *heap)
{
   ...
   if (IsHCIContextInd(hci,pInst->phones[p])) {
      ...
      anyTee=FALSEC; /* Haven't seen any final tee chains */
      for(k=0;k<hci->xc;k++) {
         ...
         if (node==NULL) {
            if (hmm->transP[1][hmm->numStates]<LSMALL) tee=FALSEC; /* Okay now */
            else tee=TRUEC; /* Still could be save by final CF models */
            /* Create new model */
            nin++;
            node=NewNode(net->heap,hmm,0);
            ...
            for (n=q+1;n<pInst->nphones;n++) {
               hmm=GetHCIModel(hci,-1,pInst->phones[n],-1);
               if (hmm->transP[1][hmm->numStates]<LSMALL) tee=FALSEC; /* Saved */
               ncf++;
               dest=NewNode(net->heap,hmm,0);
               dest->chain=pInst->chain;pInst->chain=dest;
            ...
            if (tee) anyTee=TRUEC; /* A single tee chain is too many */
      ...
      if (!anyTee) pInst->tee=FALSEC; /* Didn't see any tee chains */
      ...
      anyTee=FALSEC; /* Haven't seen any tee chains */

      for(j=0;j<hci->xc;j++) {
         if (pInst->lc[j]==NULL) continue;
         
         initTee=TRUEC; /* Start off assuming the worse */
         ...
         if (p==0) {
            ...
            for (n=1;n<p;n++) {
               ncf++;
               hmm=GetHCIModel(hci,-1,pInst->phones[n],-1);
               if (hmm->transP[1][hmm->numStates]<LSMALL) initTee=FALSEC; /* Okay now */
               dest=NewNode(net->heap,hmm,0);
               node->nlinks=1;
               nil++;
               node->links=(NetLink*) New(net->heap,
         ...
         for(k=0;k<hci->xc;k++) {
            ...
            if (node==NULL) {
               if (hmm->transP[1][hmm->numStates]<LSMALL) tee=FALSEC; /* Okay */
      ...
      if (!anyTee) pInst->tee=FALSEC; /* Didn't see any completely tee chains */
   }
}
                           ...
                           Network *net,HMMSetCxtInfo *hci,MemHeap *heap)
{
   ...
   anyTee=FALSEC; /* Haven't seen any initial tee chains */
   ...
   for(j=0;j<hci->xc;j++) {
      ...
      if (node==NULL) {
         if (hmm->transP[1][hmm->numStates]<LSMALL) tee=FALSEC; /* Okay now */
         else tee=TRUEC; /* Still could be save by initial CF models */
         nin++;
...
HMMSetCxtInfo *GetHMMSetCxtInfo(HMMSet *hset, BooleanC frcCxtInd)
{
   HMMSetCxtInfo *hci;
   LabId labid;
   MLink ml;
   ...
   if (frcCxtInd)
      labid=GetLabId("@HCI-CI@",TRUEC);
   else
      labid=GetLabId("@HCI-CD@",TRUEC);
...
Network *ExpandWordNet(MemHeap *heap,Lattice *lat,Vocab *voc,HMMSet *hset)
{
   ...
   if (!(allowXWrdExp || allowCxtExp) ||
       (forceCxtExp==FALSEC && forceRightBiphones==FALSEC &&
        forceLeftBiphones==FALSEC && ClosedDict(voc,hset))) {
      hci=GetHMMSetCxtInfo(hset,TRUEC);
      ...
      hci=GetHMMSetCxtInfo(hset,FALSEC);
      nc=hci->nc;
   }
