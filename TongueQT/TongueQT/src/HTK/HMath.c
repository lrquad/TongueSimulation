...
void WriteIntVec(FILE *f, IntVec v, BooleanC binary)
{
...
void WriteVector(FILE *f, Vector v, BooleanC binary)
{
...
void CopyMatrix(MatrixC m1, MatrixC m2)
{
   int i,nrows;
...
void Mat2DMat(MatrixC m1,  DMatrix m2)
{
   int i,j,nrows,ncols;
...
void DMat2Mat(DMatrix m1, MatrixC m2)
{
   int i,j,nrows,ncols;
...
void Mat2Tri (MatrixC m1,  TriMat m2)
{
   int i,j,nrows,ncols;
...
void Tri2Mat (TriMat m1, MatrixC m2)
{
   int i,j,nrows,ncols;
...
BooleanC ReadMatrix(Source *src, MatrixC m, BooleanC binary)
{
   int i,nrows;
...
BooleanC ReadTriMat(Source *src, TriMat m, BooleanC binary)
{
   int i,j,size;
...
void WriteMatrix(FILE *f, MatrixC m, BooleanC binary)
{
   int i,nrows;
...
void WriteTriMat(FILE *f, TriMat m, BooleanC binary)
{
   int i,j,size;
...
void ShowMatrix(char * title,MatrixC m,int maxCols,int maxRows)
{
   int i,j;
...
static BooleanC Choleski(TriMat A, DMatrix L)
{
   int size,i,j,k;
   double sum;
...
LogFloat CovInvert(TriMat c, MatrixC invc)
{
...
void LinTranQuaProd(MatrixC Prod, MatrixC A, MatrixC C)
{
   int i,j,k;
   float tempElem;
   MatrixC tempMatrix_A_mult_C;
...
/* LUDecompose: perform LU decomposition on Matrix a, the permutation
       ...
       interchanges */
static BooleanC LUDecompose(MatrixC a, int *perm, int *sign)
{
   int i,imax,j,k,n;
...
/* DLUDecompose: perform LU decomposition on Matrix a, the permutation
       ...
       depending on whether there was an even/odd number of row 
       interchanges */
static BooleanC DLUDecompose(DMatrix a, int *perm, int *sign)
{
   int i,imax,j,k,n;
   double scale,sum,xx,yy;
   DVector vv,tmp;
...
/* LinSolve: solve the set of linear equations Ax = b, returning
        the result x in  b */
static void LinSolve(MatrixC a, int *perm, float *b)
{
   int i,ii=0,ip,j,n;
   double sum;
...
float MatInvert(MatrixC c, MatrixC invc)
{
   MatrixC a;
   float col[100];
   float det;
...
double MatCofact(MatrixC c, int r, Vector cofact)
{
   DMatrix a;
   DMatrix b;
   double col[100];
...
void InitMath(void)
{
   ...
   RandInit(-1);
   minLogExp = -log(-LZERO);
   numParm = GetConfig("HMATH", TRUEC, cParm, MAXGLOBS);
   if (numParm>0){
