         ...
         if (fields[i]->occurrence == OPTIONAL)
            {
               ...
               if (ch == '*')
                  {
                     fields[i]->present = FALSEC;
                  }
               else
                  {
                     fields[i]->present = TRUEC;
                     ungetc(ch, file);
...
#define MAX_ATTR        11
   ...
   double  offsetr;
   char    **ax;
   char    got_units = FALSEC;
   char    got_scale = FALSEC;
   char    got_offset = FALSEC;
   char    got_axis_names = FALSEC;
                    ...
                    Annot     *annotate)
{
   ...
   if (depth > MAX_FLD_DEPTH)
      return FALSEC;             /* Subfield structure too deep. */
   ...
   if (rank > 0 && dim == NULL)
      return FALSEC;             /* Inconsistent rank and dimensions. */
   ...
   switch (field->occurrence)
      ...
      case INCLUDED:
         ...
         break;
      default:
         return FALSEC;          /* Bad occurrence class. */
      }
   ...
   if (field->type != NO_TYPE
       && field->occurrence != REQUIRED && field->occurrence != OPTIONAL)
      {
         ...
         if (!WriteAsciiData(field, file, annotate))
            return FALSEC;       /* Failed writing data. */
   ...
   if (subfields != NULL)
      for (i = 0; subfields[i] != NULL; i++)
         if (!WriteAsciiFieldSpec(subfields[i],
            ...
            return FALSEC;       /* Failed writing subfield spec. */
