...
static BooleanC rawMITFormat = FALSEC;    /* Don't use HTK quoting and escapes */
...
void InitLM(void)
{
   ...
   nParm = GetConfig("HLM", TRUEC, cParm, MAXGLOBS);
   if (nParm>0){
...
NEntry *GetNEntry(NGramLM *nglm,lmId ndx[NSIZE],BooleanC create)
{
   NEntry *ne;
   unsigned int hash;
   int i;
   ...
   for (j=0;j<neCnt;j++) {
      ...
      if (ne!=NULL && ne->nse>0) {
         for (i=0,se=ne->se;i<ne->nse;i++,se++) {
            ...
            if (n<nglm->nsize) be=GetNEntry(nglm,ndx,FALSEC);
            else be=NULL;
...
static int ReadNGrams(NGramLM *nglm,int n,int count, BooleanC bin)
{
   float prob;
   LabId wdid;
   SEntry *cse;
   ...
   for (g=1; g<=count; g++){
      ...
      total++;
      ne = GetNEntry(nglm,ndx+1,FALSEC);
      if (ne == NULL)
      ...
      if (bin) {
         if (flags & BIN_ARPA_HAS_BOWT) {
            ne = GetNEntry(nglm,ndx,TRUEC);
            ne->bowt = GetFloat (TRUEC)*LN10;
         }
      }
      else {
...
float GetLMProb(LModel *lm, LabId prid[NSIZE], LabId wdid)
{
   ...
   case boNGram:
      ...
      cpid[s]=0;
      ne = GetNEntry(lm->data.ngram,ndx,FALSEC);
      if (ne) {
...
LogFloat LMTrans (LModel *lm, LMState src, LabId wdid, LMState *dest)
{
   ...
   ne = (LMState) GetNEntry (nglm, hist, FALSEC);
   for ( ; !ne && (l > 0); --l) {
      hist[l] = 0;              /* back off */
      ne = (LMState) GetNEntry (nglm, hist, FALSEC);
   }
