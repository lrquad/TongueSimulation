...
static BooleanC PhoneMEE = TRUEC;  /* repeat of config also used in HFBLat.c */
#ifdef SUPPORT_QUINPHONE
static BooleanC Quinphone = FALSEC; /* repeat of config also used in HFBLat.c */
#endif
...
BooleanC IsNonSilArc(LArc *larc){ /*returns TRUE if this word is non SENT_START etc. */
...
BooleanC NonSil_and_Quinphone_IsStartPhone(LArc *larc, int i){
  if(!Quinphone){ 
...
BooleanC GetBestCorrectness /*step correctness by 1 phone.*/
  ...
  if(!cn_prev->IsSilence){
    BooleanC CorrFullPhone = FALSEC; /* counted against insertions. */
    BooleanC CorrPartialPhone = FALSEC; /* not counted against insertions. */
    int nDel = 0;
    
    /* First see if same phone between "end" and i-1, if any.*/
    for(j=end+1;j<=i-1;j++){ /*probably empty loop. */
      int p;
      BooleanC Same=FALSEC;
      for(p=0;p<niphones[j];p++){
        int this_iphone = iphone[j][p];
        if(this_iphone==iphone_prev) Same = TRUEC;
      }
      if(nonempty[j]){
        if(Same) CorrFullPhone = TRUEC; 
        nDel++;
      } else {
        if(Same) CorrPartialPhone = TRUEC; 
      }
    }
...
float DoCorrectness(FBLatInfo *fbInfo, MemHeap *mem, ArcInfo *ai, float prune, 
		    ...
		    unsigned char *nonempty, /* if TRUE then no null transition at that sausage position */
                    int T, int N, 
                    float InsCorrectness, BooleanC Quinphone, float pr_in){ 
  HArc *a;  
  ...
  if(!PhoneMEE){ 
    ...
    fbInfo->AvgCorr = fbInfo->MPEFileLength + 
      DoCorrectness(fbInfo, &fbInfo->tempStack, fbInfo->aInfo, EXACTCORR_PRUNE/*prune*/, PHONE_BEAM, 
		    minn_of_t, maxn_of_t, niwords, iwords, nonempty, fbInfo->T, nWords, InsCorrectness, FALSEC/*Quinphone*/, fbInfo->pr);
	...
	for(j=0;j<larc->nAlign;j++){ 
	  if(NonSil_and_Quinphone_IsStartPhone(larc,j)){ /* Is a starting phone... ( x_nnn_2 or some such, I think...), and is not silence. */
	    int startT, endT;
	    int nStates_quinphone, state_quinphone=0,x; BooleanC Found=FALSEC;
	    ...
	    for(x=0;x<niphones[startPos+p];x++)if(local_iphone==iphone[startPos+p][x]){ Found=TRUEC; break; } 
...
void InitExactMPE(void)
{
   ...
   nParm = GetConfig("HEXACTMPE", TRUEC, cParm, MAXGLOBS);
   if (nParm>0){
   ...
   nParm = GetConfig("HFBLAT", TRUEC, cParm, MAXGLOBS); 
   if (nParm>0){
      ...
      if (GetConfBool(cParm,nParm,"MWE",&b)) if(b) PhoneMEE=FALSEC; /* this config also used in HFBLat.c */
