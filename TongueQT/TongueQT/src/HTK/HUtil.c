...
static int trace = 0;
static BooleanC parsePhysicalHMM = FALSEC;
...
void InitUtil(void)
{
   ...
   nParm = GetConfig("HUTIL", TRUEC, cParm, MAXGLOBS);
   if (nParm>0){
...
void SetParsePhysicalHMM(BooleanC parse)
{
...
SVector CloneSVector(MemHeap *hmem, SVector s, BooleanC sharing)
{
...
SMatrix CloneSMatrix(MemHeap *hmem, SMatrix s, BooleanC sharing)
{
...
STriMat CloneSTriMat(MemHeap *hmem, STriMat s, BooleanC sharing)
{
...
MixPDF *CloneMixPDF(HMMSet *hset, MixPDF *s, BooleanC sharing)
{
   MixPDF *t;
...
MixtureVector CloneStream(HMMSet *hset, StreamElem *ste, BooleanC sharing)
{
   int m,M;
   MixtureElem *sme,*tme;
   MixtureVector mv;
...
StateInfo *CloneState(HMMSet *hset, StateInfo *ssi, BooleanC sharing)
{
...
void CloneHMM(HLink src, HLink tgt, BooleanC sharing)
{
   StateElem *s,*t;
   int i;
...
BooleanC GoNextHMM(HMMScanState *hss)
{
   int M;
   MLink mac;
...
BooleanC GoNextState(HMMScanState *hss, BooleanC noSkip)
{
   BooleanC stepping = FALSEC, ok = TRUEC;
   int M;
   
   while (IsSeen(hss->si->nUse) && ok){
      if (hss->i < hss->N-1) {
         ++hss->i; ++hss->se; stepping = TRUEC;
         hss->si = hss->se->info;
      } else if (noSkip)
         return FALSEC;
      else{
...
BooleanC GoNextStream(HMMScanState *hss, BooleanC noSkip)
{
   BooleanC stepping = FALSEC, ok = TRUEC;
   int M;
   ...
   while (IsSeen(hss->ste->nMix) && ok){
      if (hss->s < hss->S) {
         ++hss->s; ++hss->ste; stepping = TRUEC;
      } else if (noSkip)
         return FALSEC;
      else{
         stepping = FALSEC;    
         ok = GoNextState(hss,FALSEC);
      }
   }
   if (ok) {
      Touch(&hss->ste->nMix);
...
BooleanC GoNextMix(HMMScanState *hss, BooleanC noSkip)
{
   BooleanC ok = TRUEC;
...
void ConvDiagC(HMMSet *hset, BooleanC convData)
{
   HMMScanState hss;
   SVector v;
   int k;
...
void ConvLogWt(HMMSet *hset)
{
   ...
   if (hset->hsKind == DISCRETEHS || hset->hsKind == TIEDHS || hset->logWt == TRUEC) 
      return;
   NewHMMScan(hset, &hss);
   while (GoNextMix(&hss,FALSEC))
...
void ConvExpWt(HMMSet *hset)
{
   ...
   if (hset->hsKind == DISCRETEHS || hset->hsKind == TIEDHS  || hset->logWt == FALSEC) 
      return;
   NewHMMScan(hset, &hss);
   while (GoNextMix(&hss,FALSEC))
...
IntSet CreateSet(int size)
{
   ...
   s.nMembers = size;
   b = (BooleanC*) New(&setHeap, sizeof(BooleanC)*size);
   s.set = b-1;
   for (i=1;i<=size;i++) *b++ = FALSEC;
   return s;
}
...
BooleanC IsMember(IntSet s, int x)
{
   if (x<1 || x>s.nMembers)
...
BooleanC IsFullSet(IntSet s)
{
   int i;
   
   for (i=1;i<=s.nMembers;i++) 
      if (!s.set[i]) return FALSEC;
   return TRUEC;
}
...
static void PHIdent(ILink *models, HMMSet *hset)
{
   ...
   MLink q;
   LabId hmmId;
   BooleanC fullName=TRUEC; /* are there wildcards in the name */
   char *p;
   ...
   while ((*p != '\0') && (h<MAXSTRLEN) && (fullName)) {
     if ((*p=='*')||(*p=='?')||(*p=='%')) fullName=FALSEC;
     h++; 
     p = pattern+h;
   }
   if (fullName) { /* this is the name of the model */
      hmmId = GetLabId(pattern,FALSEC);
...
void LoadStatsFile(char *statfile,HMMSet *hset,BooleanC otrace)
{
   Source src;
   char hname[256];
   ...
   while(ReadInt(&src,&idx,1,bin)) {
      ...
      if ((hmmId = GetLabId(hname,FALSEC))==NULL)
