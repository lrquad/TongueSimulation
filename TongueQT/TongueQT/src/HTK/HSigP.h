...
void SpecLogModulus(Vector s, Vector m, BooleanC invert);
...
typedef struct{
   ...
   BooleanC usePower;    /* use power rather than magnitude */
   BooleanC takeLogs;    /* log filterbank channels */
...
FBankInfo InitFBank(MemHeap *x, int frameSize, long sampPeriod, int numChans,
                    float lopass, float hipass, BooleanC usePower, BooleanC takeLogs,
                    BooleanC doubleFFT,
...
void AddRegression(float *data, int vSize, int n, int step, int offset, 
                   int delwin, int head, int tail, BooleanC simpleDiffs);
/*
...
void AddHeadRegress(float *data, int vSize, int n, int step, int offset, 
                    int delwin, BooleanC simpleDiffs);
/* 
