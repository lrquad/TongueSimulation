...
static BooleanC lineOut = TRUEC;
static BooleanC phonesOut = TRUEC;
static BooleanC speakerOut = FALSEC;
static BooleanC lineIn = TRUEC;
static BooleanC micIn = FALSEC;

static volatile BooleanC stopSignalled;

typedef enum { ADS_INIT, ADS_OPEN, ADS_SAMPLING, 
               ADS_STOPPED, ADS_CLOSED } AudioDevStatus;

typedef struct {       /* circular buffer */
   BooleanC isActive;     /* true if in use */
...
typedef struct _AudioOut {
   ...
   BooleanC isActive;         /* true when device active */
   /* -- Machine Dependent Part -- */
#ifdef MMAPI_AUDIO
   MMRESULT mmError;
   DWORD magic;             /* Magic number identifying this instance */
...
BooleanC mmeFreeMem(void *ptr)
{
   ptr=GlobalFree(ptr);
   return(TRUEC);
}

BooleanC mmeFreeBuffer(void *ptr)
{
   ptr=GlobalFree(ptr);
   return(TRUEC);
}
...
BooleanC mmeCheckForCallbacks(void)
{
   return(FALSEC);
}
#endif
...
static void InitAudi(AudioIn a, HTime *sampPeriod)
{
...
#ifdef MMAPI_AUDIO 
   {
      ...
      a->callBackEvent = CreateEvent( NULL, TRUEC, FALSEC, "callBack" );
#endif
      if (trace & T_AUD) {
...
#ifdef RS6000_AUDIO
   {
      ...
      a->rc = UMSAudioDevice_enable_input(a->adevin,a->evin,a->inConn,
                                          &a->lgain,&a->rgain);
      a->rc = UMSAudioDevice_set_monitor(a->adevin,a->evin,TRUEC);
...
int AudioDevInput(int *mask) 
{
   ...
   if (mask==NULL) {
      ...
      in=*mask;
      micIn = ((in&HA_IN_MIC)?TRUEC:FALSEC);
      lineIn = ((in&HA_IN_LINE)?TRUEC:FALSEC);
   }
   return in;
}
...
int AudioDevOutput(int *mask) 
{
   ...
   if (mask==NULL) {
      ...
      out=*mask;
      speakerOut = ((out&HA_OUT_SPEAKER)?TRUEC:FALSEC);
      phonesOut = ((out&HA_OUT_PHONES)?TRUEC:FALSEC);
      lineOut = ((out&HA_OUT_LINE)?TRUEC:FALSEC);
   }
   return out;
}
...
static void CloseAudo(AudioOut a)
{
#ifdef MMAPI_AUDIO 
   {
      ...
      for(p=a->pHead;p!=NULL;p=p->next) {
         ...
         if (mmeFreeMem(p->waveData)!=TRUEC)
...
void InitAudio(void)
{
   ...
   numParm = GetConfig("HAUDIO", TRUEC, cParm, MAXGLOBS);
   if (numParm>0){
...
{
   if (a->rbuf.isActive==FALSEC) return;
...
static volatile BooleanC waitForSigH;
static volatile BooleanC alreadyFilling;
static int nz=0;
...
static void StopAndFlushAudio(AudioIn a, BooleanC deferred)
{
   if (trace&T_TOP)
...
static void FillBufferFromAudio(AudioIn a,int min)
{
   if (a->sig<0 && KeyPressed(0))
      StopAndFlushAudio(a,FALSEC);
   alreadyFilling=TRUEC;
   while(waitForSigH);
   ProtectedFillBufferFromAudio(a,min);
   if(stopSignalled)  {
      StopAndFlushAudio(a,TRUEC);
      stopSignalled = FALSEC;
   }
   alreadyFilling=FALSEC;
}
...
void StartAudioInput(AudioIn a, int sig)
{
   ...
   if (sig<0) { /* Means use keys to start/stop */
      ...
      waitForSigH = FALSEC; alreadyFilling = FALSEC;
      stopSignalled = FALSEC;
   }
   else
      StartAudioSignal();
