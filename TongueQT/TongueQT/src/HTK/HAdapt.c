...
static BooleanC saveSpkrModels = FALSEC;
...
static BooleanC useBias = TRUEC;         /* whether a bias is to be estimated for the xform */
static BooleanC storeMInfo = TRUEC;      /* whether original model information  is to be stored */
static BooleanC keepXFormDistinct = TRUEC;
static BooleanC swapXForms = FALSEC;     /* swap the transforms around after generating transform */
static BooleanC mllrCov2CMLLR= FALSEC;   /* apply mllrcov transforms as cmllr transform */ 
static BooleanC mllrDiagCov = FALSEC;    /* perform diagonal covariance adaptation */
...
static BooleanC semiTied2InputXForm = FALSEC;
static BooleanC semiTiedVFloor = TRUEC;
static int numNuisanceDim = 0;
static TriMat *avCov = NULL;
static BooleanC staticSemiTied = FALSEC;
static BooleanC initNuisanceFR = TRUEC;
static BooleanC initNuisanceFRIdent = FALSEC;
static BooleanC saveSemiTiedBinary = FALSEC;

/* specifies whether the transforms change the model variances */
static BooleanC covarChanged = FALSEC;
static BooleanC covarPChanged = FALSEC;
...
static BooleanC AccAdaptMean(AdaptXForm *xform)
{
...
static BooleanC AccAdaptVar(AdaptXForm *xform)
{
...
static BooleanC AccAdaptBaseTriMat(AdaptXForm *xform)
{
  XFormKind xkind = xform->xformSet->xkind;
...
BooleanC HardAssign(AdaptXForm *xform)
{
   AdaptKind akind = xform->akind;
   return ((akind == TREE) || (akind == BASE));
}

static BooleanC StoreObsCache(AdaptXForm *xform)
{
   XFormKind xkind = xform->xformSet->xkind;
   return ((xkind == CMLLR)  || (xkind == MLLRCOV) || (xkind == SEMIT));
}

static BooleanC StoreAdaptMean(AdaptXForm *xform)
{
   XFormKind xkind = xform->xformSet->xkind;
   return ((xkind == MLLRMEAN) || (xkind == MLLRCOV));
}

static BooleanC StoreAdaptCov(AdaptXForm *xform)
{
   XFormKind xkind = xform->xformSet->xkind;
...
void InitAdapt (XFInfo *xfinfo) 
{
   ...
   nParm = GetConfig("HADAPT", TRUEC, cParm, MAXGLOBS);
   ...
   xfinfo->useOutXForm = FALSEC;
   xfinfo->useInXForm = FALSEC;
   xfinfo->usePaXForm = FALSEC;
...
{
   MInfo *mi;
   BooleanC adaptMean, adaptCov;
   AdaptXForm *xf;
   int size;

   size = VectorSize(mp->mean);
   adaptMean = FALSEC; adaptCov = FALSEC;
   mi = (MInfo *)New(&infoStack,sizeof(MInfo));
   /* depending on the nature of the transform determines the 
      parameters to be stored */
   xf = xform;
   while (xf != NULL) {
      if (StoreAdaptMean(xf)) adaptMean = TRUEC;
      if (StoreAdaptCov(xf)) adaptCov = TRUEC;
      xf = xf->parentXForm;
   }
   if (adaptMean) {
...
static void SetMInfo(HMMSet *hset, AdaptXForm *xform)
{
   ...
   NewHMMScan(hset,&hss);
   do {
      while (GoNextState(&hss,TRUEC)) {
         while (GoNextStream(&hss,TRUEC)) {            
            if (hss.isCont)                     /* PLAINHS or SHAREDHS */
               while (GoNextMix(&hss,TRUEC)) {
                  mp = hss.mp;
...
static BooleanC CompareMInfo(HMMSet *hset, AdaptXForm *xform)
{
   BooleanC adaptMean, adaptCov;
   AdaptXForm *xf;
   HMMScanState hss;
...
static void AttachXFormInfo(HMMSet *hset)
{
   ...
   hset->attXFormInfo = TRUEC;
}

static BooleanC CompareXFormInfo(AdaptXForm *xform1, AdaptXForm *xform2)
{
   AdaptXForm *xf1, *xf2;
   BooleanC valid=FALSEC;

   if (xform1 == xform2) return TRUEC;
...
static void SetAInfo(HMMSet *hset, AdaptXForm *xform, BooleanC parent)
{
   BaseClass *bclass;
   MixPDF *mp;
   ILink i;
...
static BooleanC XFormModCovar(AdaptXForm *xform)
{
  BooleanC isModified = FALSEC;
...
static BooleanC ParseNode(RegNode *node, AdaptXForm *xform, 
			 RegTree *rtree, IntVec classes)
{
   ...
   if (node->nodeOcc > rtree->thresh) {
      ...
      if (node->numChild>0) { /* Not a terminal node */
         for (c=1;c<=node->numChild;c++)
            if (ParseNode(node->child[c], xform, rtree, lclasses)) genXForm = TRUEC;
...
static BooleanC ParseTree(RegTree *rtree, AdaptXForm *xform)
{
   int c;
   IntVec classes;
   float occ;
   ...
   if (xform->xformSet->numXForms == 0) return FALSEC;
   else return TRUEC;
}
...
static void SetObsCache(AdaptXForm *xform, BooleanC parent)
{
   MixPDF *mp;
   BaseClass *bclass;
...
static void ApplyXForm2TriMat(LinXForm *linXForm, TriMat t, MatrixC m)
{  
   int size,b,bsize;
   MatrixC A,mat;
   float tmp;
...
static void DiagApplyMat2TXForm(LinXForm *linXForm, MatrixC m, Vector v)
{  
   int size,b,bsize;
   MatrixC A;
   float tmp;
...
static LinXForm *CreateLinXForm(MemHeap *x,int vsize,IntVec blockSize)
{
   ...
   xf->vFloor = NULL;
   xf->xform = (SMatrix *)New(x,(IntVecSize(blockSize)+1)*sizeof(MatrixC));
   size = 0;
...
static BooleanC GenClassXForm(BaseClass *bclass, AdaptXForm *xform)
{
  AccStruct *accs;
  int b;
  ILink i;
...
static void MatrixMult(MatrixC m1, MatrixC m2, MatrixC m)
{  
   float tempElem;
   int i,j,k;
   MatrixC mat;
...
static BooleanC CompBlockSizes(IntVec blocks1, IntVec blocks2)
{
   int nblock1, nblock2;
   int i;
...
void SetXForm(HMMSet *hset, AdaptXForm *xform)
{
   if (!(CompareXFormInfo(hset->curXForm, xform))) {
      SetAInfo(hset,xform,FALSEC);
   } 
   SetObsCache(xform,FALSEC);  
   if (storeMInfo) {
      if (hset->attMInfo) {
...
void SetParentXForm(HMMSet *hset, AdaptXForm *xform)
{
   if (!(CompareXFormInfo(hset->parentXForm, xform))) {
      SetAInfo(hset,xform,TRUEC);
   }
   SetObsCache(xform, TRUEC);
   if (storeMInfo) {
      if (hset->attMInfo) {
         if (!CompareMInfo(hset,xform))
            UpdateMInfo(hset,xform);
...
LinXForm *CopyLinXForm(MemHeap *x, LinXForm *xf)
{
   ...
   if (xf->vFloor != NULL) {
      ...
      nxf->vFloor = NULL;
   nxf->xform = (MatrixC *)New(x,(IntVecSize(xf->blockSize)+1)*sizeof(MatrixC));
...
BooleanC GenAdaptXForm(HMMSet *hset, AdaptXForm* xform)
{
   AdaptKind akind;
   BooleanC genXForm = FALSEC;
...
/* 
   ...
   has changed
*/
BooleanC UpdateSpkrStats(HMMSet *hset, XFInfo *xfinfo, char *datafn)
{
   ...
   static int nspkr = 0;
   BooleanC resetHMMSet = FALSEC, maskMatch;
   BooleanC spkrChange = FALSEC;
