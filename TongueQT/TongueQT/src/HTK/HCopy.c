...
static BooleanC stenSet=FALSEC;   /* set if either st or en set */
...
static BooleanC chopF = FALSEC;   /* set if we should truncate files/trans */

static LabId labName = NULL;    /* name of label to extract (if set) */
static BooleanC useMLF=FALSEC;    /* set if we are saving to an mlf */
static BooleanC labF=FALSEC;      /* set if we should  process label files too */
...
void SetConfParms(void)
{
   ...
   nParm = GetConfig("HCOPY", TRUEC, cParm, MAXGLOBS);
   if (nParm>0){
...
int main(int argc, char *argv[])
{
   ...
   while (NextArg() == SWITCHARG) {
      ...
      case 's':      /* start time in seconds */
         ...
         stenSet = TRUEC; chopF = TRUEC; break;
      case 't':
         if (NextArg() != INTARG)
      ...
      case 'x':
         ...
         labName = GetLabId(GetStrArg(),TRUEC);
         if (NextArg() == INTARG)
            labRep = GetChkedInt(1,100000,s);
         chopF = TRUEC; labF = TRUEC; break;
      case 'F':
         if (NextArg() != STRINGARG)
...
BooleanC IsWave(char *srcFile)
{
   FILE *f;
   long nSamp,sampP, hdrS;
   short sampS,kind;
   BooleanC isPipe,bSwap,isWave;
